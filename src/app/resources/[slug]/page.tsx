...
function scoreProductForArticle(article: ResourceArticle, product: ReviewProduct) {
  ...
  if (articleText.includes("gate") && productText.includes("gate")) score += 8;
  if (articleText.includes("car seat") && productText.includes("car seat")) score += 8;
  if (articleText.includes("sleep") && productText.includes("crib")) score += 8;
  if (articleText.includes("toy") && productText.includes("toy")) score += 8;
  if (articleText.includes("bath") && productText.includes("bath")) score += 7;
  if (articleText.includes("stroller") && productText.includes("stroller")) score += 7;
  if (articleText.includes("anchor") && productText.includes("anchor")) score += 7;
  if (articleText.includes("outlet") && productText.includes("outlet")) score += 6;
  ...
  return [
    {
      ...
      answer: "Compare the age range, fit limits, installation requirements, recall status, included hardware, safety instructions, and whether the product matches your child and home setup.",
    },
    {
      question: "Should I buy from the article image alone?",
      answer: "No. Use the article to narrow the right product category, then open the safety checklist and retailer listing to confirm current specs, age guidance, seller details, recalls, and return policy.",
    },
    {
      question: "What is the safest first step before buying?",
      answer: "Confirm the product category fits your child's age, size, room, and use case, then check manufacturer instructions and official recall resources before purchase.",
    },
  ];
}
...
export async function generateMetadata({ params }: ArticlePageProps) {
  ...
  if (!article || !isArticlePublished(article)) {
    return {
      title: "Resource Not Found | Safer Kids",
    };
  }
  ...
  const articleJsonLd = {
    ...
    "@graph": [
      ...
      },
      {
        ...
        name: `Recommended Safer Kids shopping guides for ${article.title}`,
          ...
          <section className="mt-8 rounded-lg border border-[#dce5dc] bg-white p-6 shadow-sm">
            <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
              <div>
                ...
                <h2 className="mt-2 text-2xl font-black">Compare safety picks that fit this topic.</h2>
              </div>
