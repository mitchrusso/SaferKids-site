...
const fromEmail = process.env.CONTACT_FROM_EMAIL || "Safer Kids <onboarding@resend.dev>";
...
export async function POST(request: Request) {
  ...
  const { error } = await resend.emails.send({
    ...
    subject: `Safer Kids contact: ${subject}`,
    html: `
      <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #18211f;">
        <h2>New Safer Kids Contact Form Message</h2>
    ...
    `,
    text: `New Safer Kids Contact Form Message\n\nName: ${name}\nEmail: ${email}\nSubject: ${subject}\n\n${message}`,
  });
